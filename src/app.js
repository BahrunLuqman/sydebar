...
const handleStartupEvent = {
	win32: function() {
		...
		const AppDataDir = Path.join(process.env.LOCALAPPDATA, 'sydebar');
		if (process.argv[1] === '--squirrel-install') {
			childProc.execSync('Update.exe --createShortcut=sydebar.exe', {
				cwd: AppDataDir,
				timeout: 10000
			});
			Electron.app.quit();
			...
			if (process.platform !== 'darwin') {
				if (TrayIconFirstMinimized) {
					...
					TrayIcon.displayBalloon({
						...
						title: 'sydebar is still running',
