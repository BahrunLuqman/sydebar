...
(function() {
	sydebar.File.maxSize = 210763776;
	sydebar.File.blockSize = 131072;
	...
	var fileCrypto = {
		encrypt: function(k, iv, m) {
			...
			aes.setAAD(new Buffer('sydebar', 'utf8'));
	...
	var putFile = function(file, onProgress, onEnd) {
		...
		var blockSize = sydebar.File.blockSize;
		var blockIds = [];
		var putList = function() {
			...
			var put = HTTPS.request({
				hostname: 'sydebar.blob.core.windows.net',
				port: 443,
				protocol: 'https:',
				method: 'PUT',
				...
				console.info('sydebar.File: Azure BlockList', blockIds);
				onEnd({
					name: file.name,
		...
		var putBlock = function(offset, blockId) {
			...
			var put = HTTPS.request({
				...
				console.info(`sydebar.File: Azure Block ${blockId64}`, res.statusCode);
	...
	sydebar.File.getType = function(name) {
		...
		Object.keys(sydebar.File.types).forEach((t) => {
			if ((sydebar.File.types[t].indexOf(e) >= 0)) {
				allowed = true;
				type = t;
				ext = e;
			}
	...
	sydebar.File.parseInfo = function(infoString) {
		var parsed = {};
		try {
		...
		var fileType = sydebar.File.getType(parsed.name);
		if (
			...
			!(/(\/|\\|\~)/).test(parsed.name) &&
			sydebar.Patterns.hex64.test(parsed.url) &&
			sydebar.Patterns.hex32.test(parsed.key) &&
			sydebar.Patterns.hex12.test(parsed.iv) &&
			sydebar.Patterns.hex16.test(parsed.tag) &&
			fileType.allowed &&
			(parsed.valid === true)
		) {
			return {
	...
	sydebar.File.receive = function(info, onProgress, onEnd) {
		...
		HTTPS.get(
			`https://sydebar.blob.core.windows.net/files/${info.url}`,
			saveFile
		);
	};

	sydebar.File.send = function(
		name, file, onBegin, onProgress, onEnd
	) {
		if (!sydebar.File.getType(name).allowed) {
			sydebar.Diag.error.fileExt(name);
			return false;
		}
		if (file.length > sydebar.File.maxSize) {
			sydebar.Diag.error.fileMaxSize(name);
			onBegin({
				name: name,
				url: '',
				key: '',
		...
		sydebar.Pinning.get(
			`https://${sydebar.Hostname}/sas`,
			function(res, valid) {
				if (!valid) {
					sydebar.Diag.error.fileGeneral(name);
					onBegin({
						name: name,
						url: '',
						key: '',
				...
				res.on('end', function() {
					if (!sydebar.Patterns.fileSas.test(sas)) {
						sydebar.Diag.error.fileGeneral(name);
						onBegin({
							name: name,
							url: '',
