...
window.addEventListener('load', function(e) {
			...
			submenu: [{
				...
				click: function() {
					Remote.shell.openExternal(
						`https://${sydebar.Hostname}/help.html`
					);
				}
			}, /*{
				label: 'Developer',
				...
				click: function() {
					Remote.shell.openExternal(
						'https://github.com/sydebar/sydebar/issues/'
					);
				}
			}, {
	...
	var checkIfSticker = function(message) {
		if (sydebar.Patterns.sticker.test(message)) {
			var stickers = [
	...
	var checkIfFile = function(message) {
		if (sydebar.Patterns.file.test(message)) {
			var info = sydebar.File.parseInfo(message);
			if (info.valid) {
				return {
					file:	info,
					isFile: true
	...
	var chatMessage = React.createClass({
		...
		getInitialState: function() {
			return {
				sending: (
					this.props.sender === sydebar.Me.username
				)
			};
		},
		componentDidMount: function() {
	...
	var chatFileDragOverlay = React.createClass({
		...
		render: function() {
			...
			'Your file will be encrypted and sent over sydebar.'));
		}
	});
	...
	var chatWindow = React.createClass({
		...
		updateConversation: function(fromMe, info) {
			...
			if (sticker.isSticker) {
				res = React.createElement(chatSticker, {
					key: this.state.key,
					alignment: alignment,
					timestamp: sydebar.Time.getTimestamp(info.stamp),
		...
		record: function(e) {
			...
			if (!this.state.recordVisible) {
				sydebar.Recording.createStream(function(stream) {
				...
				sydebar.Recording.stop(function(video) {
					URL.revokeObjectURL(
		...
		sendFile: function(name, file) {
			...
			sydebar.File.send(name, file, function(info) {
				if (!info.valid) {
					return false;
				}
				var sendInfo = 'sydebarFile:' + JSON.stringify(info);
				...
				if (info.valid) {
					...
					sydebar.Diag.error.fileGeneral(info.name);
					return false;
				}
		...
		receiveFile: function(file) {
			var _t = this;
			sydebar.File.receive(file, function(url, p) {
				_t.files[url].setState({progress: p});
				Remote.getCurrentWindow().setProgressBar(p / 100);
	...
	IPCRenderer.once('chat.init', function(e, data) {
		sydebar.Me.username = data.myUsername;
		thisChat.window.setState({
			status: data.status,
			avatar: data.avatar,
	...
	IPCRenderer.on('chat.messageError', function(e, internalId) {
		...
		sydebar.Diag.error.messageSending();
	});
	...
	IPCRenderer.on('chat.noDevicesError', function(e, internalId) {
		...
		sydebar.Diag.error.noDevices(thisChat.window.state.to);
	});
	...
	window.addEventListener('beforeunload', function(e) {
		...
		var unsavedFilesDiag = function() {
			sydebar.Diag.message.unsavedFiles(function(response) {
				if (response === 1) {
		...
		var unsentFilesDiag = function() {
			sydebar.Diag.message.unsentFiles(function(response) {
				if (response === 1) {
					Remote.getCurrentWindow().destroy();
				}
			});
		...
		Object.keys(thisChat.window.files).forEach((file) => {
			...
			if (
				(thisChat.window.files[file].props.sender !== sydebar.Me.username) &&
		...
		if (thisChat.sendQueue.messages.length) {
			sydebar.Diag.error.messagesQueued(
