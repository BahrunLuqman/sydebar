...
window.addEventListener('load', function(e) {
	...
	sydebar.Update.saveDialog(Remote.getCurrentWindow(), function(path) {
		if (!path) {
		...
		var onError = function() {
			sydebar.Diag.error.updateDownloader();
			FS.unlink(path);
			setInterval(function() {
				Remote.getCurrentWindow().close();
			}, 250);
		...
		sydebar.Pinning.get(
			sydebar.Update.clientURIs[process.platform],
			function(res, valid) {
				if (!valid) {
					return onError();
				}
				...
				file.on('finish', function() {
					...
					sydebar.Update.verifySignature(hash.digest('hex'), function(valid) {
						if (valid) {
							sydebar.Diag.message.updateDownloaded(function() {
